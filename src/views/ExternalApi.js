...
export const ExternalApiComponent = () => {
  ...
  const getUser = async () => {
    try {
      ...
      const response = await fetch(`${apiOrigin}/api/get-full-id/${user.sub}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setState({
        ...
        showResult: true,
        apiMessage: responseData,
        featureFlagged: responseData.msg.featureflagged,
      });
      console.log(state.featureFlagged + " - featureFlagged");
    } catch (error) {
      setState({
        ...state,
        error: error.error,
  ...
  return (
    <>
      <div className="mb-5">
        ...
        {state.featureFlagged ? <Button color="success" className="mt-5">Flagged</Button> : <Button color="danger" className="mt-5">Not Flagged</Button>}
      </div>
